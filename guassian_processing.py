...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 28 19:10:26 2021
...
def band_to_color(inp):
    labels = [481.39, 642.18, 621, 754, 870, 1004]
    # labels = [0,1,2,3,4,5]
...
def band_to_wvlg(inp):
    labels = [0,1,2,3,4,5]
    labels_2=[481.39, 642.18, 621.5, 754.5, 870.75, 1004.0]
    outlst = []
    for x in inp:
        out = labels.index(int(x))
        out = labels_2[out]
...
def gaussian_regression(light_curve_data, maximum_sn_length =200 , n_samples = 100):
    #if data is None:
    ...
    for i in SN_uqlst:
        ...
        b = SNdf['band'].unique() == np.array([0, 1])
